...
import { adminFetch, adminInsert, adminUpdate, adminDelete } from "@/lib/admin-fns";
...
export function CrudTable<T extends { id: string } & Record<string, unknown>>({
  ...
  const { data, isLoading } = useQuery({
    queryKey,
    queryFn: () =>
      adminFetch({ data: { table, orderBy, ascending } }) as Promise<T[]>,
  });
  ...
  async function save(values: Record<string, unknown>, id?: string) {
    try {
      const clean = Object.fromEntries(
        Object.entries(values).filter(([k, v]) => k !== "id" && v !== "" && v !== undefined)
      );
      if (id) {
        await adminUpdate({ data: { table, id, data: clean } });
        toast.success("Updated");
      } else {
        await adminInsert({ data: { table, data: clean } });
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          onClick={startCreate}
          className="inline-flex items-center gap-2 rounded-xl bg-[var(--medical)] px-4 py-2 text-sm font-semibold text-white"
        >
    ...
    <div className="mb-4 rounded-2xl border bg-card p-5 shadow-soft">
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-lg bg-[var(--medical)] px-3 py-1.5 text-sm text-white"
        >
